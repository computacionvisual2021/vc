    ...
    for (i = 0; i <= squares; i++) {
        ...
        for (j = 0; j <= squares; j++) {
            ...
            if (inside_triangle(ax, ay, bx, by, cx, cy, col + pointA, row + pointA)) {
                ...
                let coord = [col + pointA / 2, row + pointA / 2, col + (3 * pointA / 2), row + pointA / 2, col + pointA / 2, row + (3 * pointA / 2), col + (3 * pointA / 2), row + (3 * pointA / 2) ];
                //let origin = [col, row, col + pointA, row, col, row + pointA , col + pointA, row + pointA];
                //var rect1 = (col + pointA / 2, row + pointA/2);
                //var rect2 = (col + (3 * pointA / 2), row + pointA / 2);
                //var rect3 = (col + pointA / 2, row + (3 * pointA / 2));
                //var rect4 = (col + (3 * pointA / 2), row + (3 * pointA / 2));

                for (k = 0; k < 4; k++) { 
                    if (inside_triangle(ax, ay, bx, by, cx, cy, coord[k*2], coord[k*2+1])) {
                        //Calcular las coordenadas baricentricas del punto medio de la celda
                        barycentric(ax, ay, bx, by, cx, cy, coord[k * 2], coord[k * 2 + 1]);
                        //Rellenar la celda con la interpolacion entre las coordenadas baricentricas y los colores definidos
                        fill(239 * alpha, 247 * beta, 255 * gamma);
                        //Dibujar la celda recien analizada
                        rect(coord[k * 2] - pointA / 2, coord[k * 2 + 1] - pointA / 2, squaresF/2, squaresF/2);
                    } else {
